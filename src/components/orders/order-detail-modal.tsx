      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto bg-card border border-border w-[calc(100%-2rem)] sm:w-full rounded-[15px]">
        <DialogHeader className="border-b border-border pb-4">
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
            ...
            {/* Quick assign — only when unassigned and admin */}
            {isAdmin && !order.disenadoPor && (
              <div ref={assignRef} className="relative">
                <button
                  onClick={() => setAssignOpen(!assignOpen)}
                  className="flex items-center gap-2 px-4 py-2.5 rounded-[10px] font-semibold text-sm bg-[#1E293B] border border-[#334155] text-[#94A3B8] hover:text-white hover:border-[#F97316]/50 transition-all duration-200"
                >
                  <UserPlus className="w-4 h-4" />
                  Asignar
                  <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-200 ${assignOpen ? 'rotate-180' : ''}`} />
                </button>
                {assignOpen && (
                  <div className="absolute right-0 top-full mt-2 w-40 bg-card border border-border rounded-[10px] shadow-xl z-50 overflow-hidden">

...
function orderToRow(order: Order): Record<string, any> {
  return {
    ...
    tipo_orden: order.tipoOrden || "Rotulacion",
    // Sellos
    tipo_sello: order.tipoSello || null,
    modelo_sello: order.modeloSello || null,
    espacio_diseno: order.espacioDiseno || null,
    // Sellos + Papelería
    descuento_aplicado: order.descuentoAplicado || false,
    descuento_monto: order.descuentoMonto || 0,
    // Papelería
    trabajo: order.trabajo || null,
    factura_no: order.facturaNo || null,
    facturado_el: order.facturadoEl ? new Date(order.facturadoEl).toISOString() : null,
    hecha_por: order.hechaPor || null,
    atendido_por: order.atendidoPor || null,
    codigo_cliente: order.codigoCliente || null,
    muestra: order.muestra || false,
    color_tinta: order.colorTinta || null,
    color_tinta_otro: order.colorTintaOtro || null,
    tipo_arte: order.tipoArte || null,
    numerado_por: order.numeradoPor || null,
...
function rowToOrder(row: any): Order {
  ...
  return {
    ...
    tipoOrden: (row.tipo_orden as any) || "Rotulacion",
    // Sellos
    tipoSello: row.tipo_sello || undefined,
    modeloSello: row.modelo_sello || undefined,
    espacioDiseno: row.espacio_diseno || undefined,
    // Sellos + Papelería
    descuentoAplicado: row.descuento_aplicado || false,
    descuentoMonto: row.descuento_monto ? Number(row.descuento_monto) : undefined,
    // Papelería
    trabajo: row.trabajo || undefined,
    facturaNo: row.factura_no || undefined,
    facturadoEl: row.facturado_el ? new Date(row.facturado_el) : undefined,
    hechaPor: row.hecha_por || undefined,
    atendidoPor: row.atendido_por || undefined,
    codigoCliente: row.codigo_cliente || undefined,
    muestra: row.muestra || false,
    colorTinta: row.color_tinta || undefined,
    colorTintaOtro: row.color_tinta_otro || undefined,
    tipoArte: row.tipo_arte || undefined,
    numeradoPor: row.numerado_por || undefined,
